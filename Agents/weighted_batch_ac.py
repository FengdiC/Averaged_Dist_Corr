...
class WeightedBatchActorCritic(A):
    ...
    def update(self,closs_weight):
        ...
        pobj = self.new_lprobs * (returns - self.values).detach() *self.weights.detach()/self.args.scale_weight
    ...
    def update_weight(self,scale=1.0):
        ...
        self.wloss = torch.mean((torch.from_numpy(self.labels).to(self.device)*self.args.scale_weight-self.weights)**2)
...
class SharedWeightSeparateUpdateBatchAC(A):
    def __init__(self, lr, gamma, BS, o_dim, n_actions, hidden, args, device=None, shared=False, continuous=False):
        super(SharedWeightSeparateUpdateBatchAC, self).__init__(lr=lr, gamma=gamma, BS=BS, o_dim=o_dim, n_actions=n_actions,
                                                          hidden=hidden, args=args, device=device, shared=shared)
        self.args = args
        self.continuous = continuous
    ...
    def update_weight(self,scale=1.0):
        # input: data  Job: finish one round of gradient update
        _, self.weights = self.weight_critic(torch.from_numpy(self.frames).to(self.device))
        self.labels = self.gamma**self.times
    ...
    def update(self, closs_weight):
        # input: data  Job: finish one round of gradient update
        self.next_values, _= self.weight_critic(torch.from_numpy(self.next_frames).to(self.device))
        self.values, self.weights = self.weight_critic(torch.from_numpy(self.frames).to(self.device))
        self.new_lprobs, _ = self.network(torch.from_numpy(self.frames).to(self.device),
                                          torch.from_numpy(self.actions).to(self.device))

        self.dones = torch.from_numpy(self.dones).to(self.device)
        returns = torch.from_numpy(self.rewards).to(self.device) + (
                    1 - self.dones) * self.gamma * self.next_values.detach()
